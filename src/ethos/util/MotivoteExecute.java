	...
	public static void run(Player player, String auth) {
		...
		VOTE.execute(() -> {
			try {
				...
				if (success) {
					authAmount++;
					if (!player.getItems().addItem(1464, player.getRights().isOrInherits(Right.DIAMOND_CLUB) ? 2 : Config.DOUBLE_VOTE_INCENTIVES ? 2 : 1)) {
						Server.itemHandler.createGroundItem(player, 1464, player.getX(), player.getY(), player.heightLevel, player.getRights().isOrInherits(Right.DIAMOND_CLUB) ? 2 : Config.DOUBLE_VOTE_INCENTIVES ? 2 : 1);
					}
