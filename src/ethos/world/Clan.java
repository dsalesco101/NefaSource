	...
	public void sendChat(Player paramClient, String paramString) {
		...
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				...
				if (c != null && activeMembers.contains(c.playerName)) {
					if (!c.getIgnores().has(paramClient.getNameAsLong()) || paramClient.getRights().hasStaffPosition()) {
						c.sendMessage("/@bla@[@blu@" + getTitle() + "@bla@] " + paramClient.getRights().getCrowns() + "@bla@"
								+ Misc.optimizeText(paramClient.playerName) + ": @dre@"
