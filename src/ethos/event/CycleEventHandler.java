...
public class CycleEventHandler {
	...
	public CycleEventContainer addEvent(Object owner, CycleEvent event, int cyclesBetweenExecution) {
		return addEvent(-1, owner, event, cyclesBetweenExecution);
	}
	...
	public CycleEventContainer addEvent(int id, Object owner, CycleEvent event, int cycles, boolean randomized) {
		CycleEventContainer cycleEventContainer = new CycleEventContainer(id, owner, event, cycles, randomized);
	...
	public boolean isAlive(Object owner) {
		Optional<CycleEventContainer> op = events.stream().filter(Objects::nonNull).filter(container -> container.getOwner().equals(owner)).findFirst();
		return op.isPresent();
	}
