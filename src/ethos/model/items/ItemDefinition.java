...
public class ItemDefinition {
	...
	public static void load() throws IOException {
		...
		try(FileReader fr = new FileReader(new File("./Data/json/item_definitions.json"))) {
			...
			list.forEach(item -> definitions.put(item.id, item));
		} catch(Exception ex) {
			ex.printStackTrace();
		}

		System.out.println("Loaded " + definitions.size() + " item definitions. Max ID: " + (definitions.keySet().stream().max(Integer::compareTo)));
	}
