...
import ethos.model.content.lootbag.LootingBagItem;
...
public class ItemAssistant {
	...
	private static final String DEFAULT_WEAPON_NAME = "Unarmed";
	private static final int FIGHT_MODE_CONFIG = 43;
	private static final int FIGHT_MODE_ACCURATE = 0;
	private static final int FIGHT_MODE_BLOCK = 1;
	private static final int FIGHT_MODE_AGGRESSIVE = 2;
	private static final int FIGHT_MODE_CONTROLLED = 3;
	...
	public void processContainerUpdates() {
		while (!containerUpdates.isEmpty()) {
			...
			case INVENTORY:
				c.getRunePouch().sendPouchRuneInventory();
				resetItems(3214); // Standard inventory
				resetItems(3823); // Inventory within shop interface
				break;
			case EQUIPMENT:
	...
	public void resetItems(int WriteFrame) {
		if (c.getOutStream() != null && c != null) {
			...
			c.getOutStream().writeUnsignedWord(WriteFrame);
			c.getOutStream().writeUnsignedWord(c.playerItems.length);
			for (int i = 0; i < c.playerItems.length; i++) {
				if (c.playerItemsN[i] > 254) {
					c.getOutStream().writeByte(255);
					c.getOutStream().writeDWord_v2(c.playerItemsN[i] == -1 ? 0 : c.playerItemsN[i]);
				} else {
					c.getOutStream().writeByte(c.playerItemsN[i] == -1 ? 0 : c.playerItemsN[i]);
				}
	...
	public void sendItemContainer(int interfaceId, List<? extends GameItem> container, boolean addToItemId) {
		if (c.getOutStream() != null && c != null) {
			...
			c.getOutStream().writeUnsignedWord(container.size());
			for (int i = 0; i < container.size(); i++) {
				...
				if (item == null) {
					...
					if (container.get(i).getAmount() > 254) {
						c.getOutStream().writeByte(255);
						c.getOutStream().writeDWord_v2(container.get(i).getAmount() == -1 ? 0 : container.get(i).getAmount());
					} else {
						c.getOutStream().writeByte(container.get(i).getAmount() == -1 ? 0 : container.get(i).getAmount());
					}

					c.getOutStream().writeWordBigEndianA(container.get(i).getId() + (addToItemId ? 1 : 0));
				}
			}
			c.getOutStream().endFrameVarSizeWord();
			c.flushOutStream();
	...
	public void addItemToBank(int itemId, int amount) {
		...
		if (c.getMode().isUltimateIronman()) {
			if (0 < c.getItems().freeSlots()) {
				Server.itemHandler.createGroundItem(c, itemId, c.absX, c.absY, c.heightLevel, amount, c.getIndex());
				c.sendMessage("Your items was dropped on the floor due to your mode and no inventory room.");
				return;
			}
			c.getItems().addItem(itemId, amount);
			c.sendMessage("Because you are ultimate ironman, your items went into your inventory.");
			return;
		}
		if (tab.freeSlots() == 0) {
	...
	public void sendItemsKept() {
		...
		if (c.getOutStream() != null && c != null) {
			c.getOutStream().createFrameVarSizeWord(53);
			c.getOutStream().writeUnsignedWord(6963);
			c.getOutStream().writeUnsignedWord(c.itemKeptId.length);
	...
	public void dropAllItems() {
		...
		PlayerLogging.write(PlayerLogging.LogType.DEATH, c, String.format("Dropped %s at %s", droppedItems, c.getPosition()));
	...
	public boolean addItem(int item, long amount) {
		// synchronized(c) {
		...
		if ((((freeSlots() >= 1) || playerHasItem(item, 1)) && Item.itemStackable[item]) || ((freeSlots() > 0) && !Item.itemStackable[item])) {
			...
			this.addContainerUpdate(ContainerUpdate.INVENTORY);
			c.sendMessage("Not enough space in your inventory. " + Server.itemHandler.getItemList(item).itemName);
			return false;
		}
		// }
	}
	...
	private String getWeaponNamedFormatted(int weapon) {
		String name = getItemName(weapon);
		if (weapon == -1 || name == null || name.length() == 0) {
			return DEFAULT_WEAPON_NAME;
		} else {
			name = name.replaceAll("Bronze", "");
			name = name.replaceAll("Iron", "");
			name = name.replaceAll("Steel", "");
			name = name.replaceAll("Black", "");
			name = name.replaceAll("Mithril", "");
			name = name.replaceAll("Adamant", "");
			name = name.replaceAll("Rune", "");
			name = name.replaceAll("Granite", "");
			name = name.replaceAll("Dragon", "");
			name = name.replaceAll("Drag", "");
			name = name.replaceAll("Crystal", "");
			name = name.trim();
			return name;
		}
	}

	public void setFightMode(int newFightMode) {
		c.fightMode = newFightMode;
		updateFightModeConfig();
	}

	public void updateFightModeConfig() {
		WeaponFightMode mode = getFightMode(c.playerEquipment[3]);
		int configValue = c.fightMode;

		switch (mode) {
			case FOUR_STANDARD:
				if (c.fightMode == FIGHT_MODE_BLOCK)
					configValue = 3;
				if (c.fightMode == FIGHT_MODE_AGGRESSIVE)
					configValue = 1;
				if (c.fightMode == FIGHT_MODE_CONTROLLED)
					configValue = 2;
				break;
			case THREE_AGGRESSIVE:
				if (c.fightMode == FIGHT_MODE_AGGRESSIVE)
					configValue = 1;
				if (c.fightMode == FIGHT_MODE_BLOCK)
					configValue = 2;
		...
		c.getPA().sendConfig(FIGHT_MODE_CONFIG, configValue);
	}

	private void updateFightMode(int previousWeapon, int currentWeapon) {
		WeaponFightMode previous = getFightMode(previousWeapon);
		WeaponFightMode current = getFightMode(currentWeapon);

		if (current != previous) {
			if (previous == WeaponFightMode.THREE_CONTROLLED && current == WeaponFightMode.THREE_AGGRESSIVE
					&& c.fightMode == FIGHT_MODE_CONTROLLED) {
				setFightMode(FIGHT_MODE_AGGRESSIVE);
			} else if (previous == WeaponFightMode.THREE_AGGRESSIVE && current == WeaponFightMode.THREE_CONTROLLED
					&& c.fightMode == FIGHT_MODE_AGGRESSIVE) {
				setFightMode(FIGHT_MODE_CONTROLLED);
			} else if (previous == WeaponFightMode.FOUR_STANDARD) {
				if (current == WeaponFightMode.THREE_AGGRESSIVE && c.fightMode == FIGHT_MODE_CONTROLLED) {
					setFightMode(FIGHT_MODE_AGGRESSIVE);
				} else if (current == WeaponFightMode.THREE_CONTROLLED && c.fightMode == FIGHT_MODE_AGGRESSIVE) {
					setFightMode(FIGHT_MODE_CONTROLLED);
				}
			}
		}
	...
	private WeaponFightMode getFightMode(int weapon) {
		String name = getItemName(weapon);
		String nameFormatted = getWeaponNamedFormatted(weapon);

		if (name.endsWith("whip") || name.contains("tentacle") || nameFormatted.startsWith("halberd")) {
			return WeaponFightMode.THREE_CONTROLLED;
		} else if (
				name.endsWith("bow")
				|| name.contains("shortbow (i)")
				|| name.endsWith("10")
				|| name.endsWith("crystal")
				|| name.endsWith("bow full")
				|| name.startsWith("seercull")
				|| name.contains("blowpipe")
				|| name.contains("ballista")
				|| name.contains("thrownaxe")
				|| name.contains("chinchompa")
				|| nameFormatted.startsWith("dart")
				|| nameFormatted.startsWith("knife")
				|| nameFormatted.startsWith("javelin")
				|| name.equalsIgnoreCase("toktz-xil-ul")
				|| name.equals("Staff of the dead")
				|| name.equals("Toxic staff of the dead")
				|| name.equals("Scythe of vitur")
				|| nameFormatted.startsWith("Scythe")
				|| nameFormatted.contains("warhammer")
				|| c.playerEquipment[c.playerWeapon] == 21003
				|| c.playerEquipment[c.playerWeapon] == 4153
				|| c.playerEquipment[c.playerWeapon] == 13263
				|| c.playerEquipment[c.playerWeapon] == 12848
				...
				|| name.endsWith("staff")
				|| name.contains("Void Knight Mace")
				|| c.itemId == 22555 || name.equals("Slayer's staff (e)") || name.endsWith("of the seas") || name.endsWith("wand"))
				&& !name.contains("dead") || name.equals("Ancient staff") || name.equals("Thammaron's sceptre")
				// End conditional
		) {
			return WeaponFightMode.THREE_AGGRESSIVE;
		} else {
			return WeaponFightMode.FOUR_STANDARD;
		}
	}
	...
	public void sendWeapon(int weapon) {
		...
		if (name.equals("Unarmed")) {
			...
			c.getPA().sendFrame126(name, 5857);
		} else if (name.endsWith("whip") || name.contains("tentacle")) {
			c.setSidebarInterface(0, 12290); // flick, lash, deflect
			c.getPA().sendFrame246(12291, 200, weapon);
			c.getPA().sendFrame126(name, 12293);
		} else if (name.endsWith("bow") || name.contains("shortbow (i)") || name.endsWith("10") || name.endsWith("crystal") || name.endsWith("bow full")
				|| name.startsWith("seercull") || name.contains("blowpipe") || name.contains("ballista") || name.contains("thrownaxe") || name.contains("chinchompa")) {
			c.setSidebarInterface(0, 1764); // accurate, rapid, longrange
			c.getPA().sendFrame246(1765, 200, weapon);
			c.getPA().sendFrame126(name, 1767);
		} else if ((name.startsWith("Staff") || name.endsWith("staff") || name.contains("Void Knight Mace") || c.itemId == 22555 || name.equals("Slayer's staff (e)") || name.endsWith("of the seas") || name.endsWith("wand"))
				&& !name.contains("dead") || name.equals("Ancient staff") || name.equals("Thammaron's sceptre")) {
			c.setSidebarInterface(0, 328);
			c.getPA().sendFrame246(329, 200, weapon);
			c.getPA().sendFrame126(name, 355);				
		} else if (nameFormatted.startsWith("dart") || nameFormatted.startsWith("knife") || nameFormatted.startsWith("javelin") || name.equalsIgnoreCase("toktz-xil-ul")) {
			c.setSidebarInterface(0, 4446); // accurate, rapid, longrange
			c.getPA().sendFrame246(4447, 200, weapon);
			c.getPA().sendFrame126(name, 4449);
		} else if (nameFormatted.startsWith("dagger") || nameFormatted.startsWith("ghrazi") || nameFormatted.startsWith("darklight") || nameFormatted.startsWith("arclight") || nameFormatted.contains("anchor") || nameFormatted.contains("sword") || nameFormatted.contains("byssal dagger")) {
			c.setSidebarInterface(0, 2276); // stab, lunge, slash, block
			c.getPA().sendFrame246(2277, 200, weapon);
			c.getPA().sendFrame126(name, 2279);
		} else if (nameFormatted.startsWith("pickaxe")) {
			c.setSidebarInterface(0, 5570); // spike, impale, smash, block
			c.getPA().sendFrame246(5571, 200, weapon);
			c.getPA().sendFrame126(name, 5573);
		} else if (nameFormatted.startsWith("axe") || nameFormatted.startsWith("battleaxe")) {
			c.setSidebarInterface(0, 1698); // chop, hack, smash, block
			c.getPA().sendFrame246(1699, 200, weapon);
			c.getPA().sendFrame126(name, 1701);
		} else if (nameFormatted.startsWith("halberd")) {
			c.setSidebarInterface(0, 8460); // jab, swipe, fend
			c.getPA().sendFrame246(8461, 200, weapon);
			c.getPA().sendFrame126(name, 8463);
		} else if (name.equals("Staff of the dead") || name.equals("Toxic staff of the dead")) {
			c.setSidebarInterface(0, 28500);
			c.getPA().sendFrame246(28500, 200, weapon);
			c.getPA().sendFrame126(name, 355);
		} else if (name.equals("Scythe of vitur")) {
			c.setSidebarInterface(0, 8460); // jab, swipe, fend
			c.getPA().sendFrame246(7763, 200, weapon);
			c.getPA().sendFrame126(name, 7765);
		} else if (nameFormatted.startsWith("Scythe")) {
			c.setSidebarInterface(0, 8460); // jab, swipe, fend
			c.getPA().sendFrame246(8461, 200, weapon);
			c.getPA().sendFrame126(name, 8463);
		} else if (nameFormatted.startsWith("spear") || c.playerEquipment[c.playerWeapon] == 22610 ||
				nameFormatted.toLowerCase().contains("zamorakian") || nameFormatted.toLowerCase().contains("zamorakian hasta")) {
			c.setSidebarInterface(0, 4679); // lunge, swipe, pound, block
			c.getPA().sendFrame246(4680, 200, weapon);
			c.getPA().sendFrame126(name, 4682);
		} else if (nameFormatted.toLowerCase().contains("mace") || c.playerEquipment[c.playerWeapon] == 23528) {
			c.setSidebarInterface(0, 3796);
			c.getPA().sendFrame246(3797, 200, weapon);
			c.getPA().sendFrame126(name, 3799);
		} else if (nameFormatted.contains("warhammer") || c.playerEquipment[c.playerWeapon] == 21003  || c.playerEquipment[c.playerWeapon] == 4153 || c.playerEquipment[c.playerWeapon] == 13263 || c.playerEquipment[c.playerWeapon] == 12848 || c.playerEquipment[c.playerWeapon] == 13902) {
			c.setSidebarInterface(0, 425); // war hammer equip.
			c.getPA().sendFrame246(426, 200, weapon);
			c.getPA().sendFrame126(name, 428);
		} else if (nameFormatted.equals("claws")) {
			...
			c.getPA().sendFrame126(name, 2426);
		}
	}

	private void updateExperienceMode() {
	...
	public void addSpecialBar(int weapon, boolean sendWeapon) {
		...
		case 20784:
			...
			specialAmount(weapon, c.specAmount, 7812, sendWeapon);
			break;
		case 4151: // whip
		case 6746: //darklight
		case 12773:
		...
		case 12006:
			...
			specialAmount(weapon, c.specAmount, 12335, sendWeapon);
			break;
		...
		case 20849:
			...
			specialAmount(weapon, c.specAmount, 7561, sendWeapon);
			break;

		case 4587: // dscimmy
			c.getPA().sendFrame171(0, 7599);
			specialAmount(weapon, c.specAmount, 7611, sendWeapon);
			break;

		case 3204: // d hally
		case 13092:
			c.getPA().sendFrame171(0, 8493);
			specialAmount(weapon, c.specAmount, 8505, sendWeapon);
			break;
		...
		case 11889:
			...
			specialAmount(weapon, c.specAmount, 7686, sendWeapon);
			break;
		...
		case 21009: //Dragon sword
			...
			specialAmount(weapon, c.specAmount, 7586, sendWeapon);
			break;
		...
		case 11061:
			...
			specialAmount(weapon, c.specAmount, 7636, sendWeapon);
			break;
	...
	public void specialAmount(int weapon, double specAmount, int barId) {
		specialAmount(weapon, specAmount, barId, true);
	}
	...
	public boolean canEquip(int wearID) {
		...
		case 12902:
			if(c.playerLevel[0] < 75) {
				c.sendMessage("You must have a attack level of at least 75 to weild the toxic staff (uncharged)");
				return false;
			}
			if(c.playerLevel[6] < 75) {
				c.sendMessage("You must have a magic level of at least 75 to weild the toxic staff (uncharged)");
				return false;
			}
			break;
		case 13108:
		...
		case 13111:
			if(c.playerLevel[5] < 0) {
				c.sendMessage("contact staff if you see this message.");
				//the swords was saying needing 60 prayer but wasnt in files anywhere or json, this fixes it.
				return false;
			}
			break;
		case 20727:
			if(c.playerLevel[18] < 55) {
				c.sendMessage("You must have a slayer level of at least 55 to weild the Leaf-bladed battleaxe");
				return false;
			}
			if(c.playerLevel[0] < 65) {
				c.sendMessage("You must have a attack level of at least 65 to weild the Leaf-bladed battleaxe");
				return false;
			}
			break;
		case 21003:
		...
		case 22555:
			if(c.playerLevel[6] < 60) {
				c.sendMessage("You must have a magic level of at least 60 to weild the Thammaron's sceptre");
				return false;
			}
			break;
		case 773:
			if(!c.getRights().isOrInherits(Right.OWNER)) {
				c.sendMessage("You must be an owner to wear this.");
				return false;
			}
			break;
		case 12249: //its bugged in game and needs removing, so it deletes out inventory if old people still have it
			c.getItems().deleteItem(12249, 28);
			c.getItems().addItem(995, 200000);
			c.sendMessage("@red@This mask was bugged and got deleted.");
			c.sendMessage("@red@You have been awarded 200k coins..");
			break;
		case 21817:
			if(c.ether <=0) {
				c.sendMessage("@red@You have no ether in your bracelet.");
				return false;
			}
			break;
		case 13235:
		...
		case 7927:
			...
			c.npcId2 = wearID == 7927 ? 5542 : 5541;
			c.isNpc = true;
			c.setUpdateRequired(true);
			c.morphed = true;
			c.setAppearanceUpdateRequired(true);
	...
	public boolean wearItem(int wearID, int slot) {
		int previousWeapon = c.playerEquipment[3];
		c.getSkilling().stop();
		// synchronized (c) {
		int targetSlot = 0;
		...
		if (c.playerItems[slot] == (wearID + 1)) {
			...
			if (slot >= 0 && wearID >= 0) {
				...
				c.getItems().addContainerUpdate(ContainerUpdate.EQUIPMENT);
			}
			
			// Update equipment slot
			if (c.getOutStream() != null && c != null) {
				c.getOutStream().createFrameVarSizeWord(34);
				c.getOutStream().writeUnsignedWord(1688);
				c.getOutStream().writeByte(targetSlot);
				c.getOutStream().writeUnsignedWord(wearID + 1);
			...
			if (previousWeapon != c.playerEquipment[3]) {
				updateFightMode(previousWeapon, c.playerEquipment[3]);
			}
	...
	public void updateSlot(int slot) {
		...
		if (c.getOutStream() != null && c != null) {
			...
			c.getOutStream().writeUnsignedWord(c.playerEquipment[slot] + 1);
		...
		if (c.getOutStream() != null && c != null) {
			if (c.playerEquipment[slot] > -1) {
				if (addItem(c.playerEquipment[slot], c.playerEquipmentN[slot])) {
					...
					c.getOutStream().writeUnsignedWord(6);
					c.getOutStream().writeUnsignedWord(1688);
					c.getOutStream().writeByte(slot);
	...
	public void itemOnInterface(int id, int amount) {
		// synchronized (c) {
		c.getOutStream().createFrameVarSizeWord(53);
		c.getOutStream().writeUnsignedWord(2274);
		c.getOutStream().writeUnsignedWord(1);
		if (amount > 254) {
			c.getOutStream().writeByte(255);
	...
	public void resetBankInstant() {
		// Shift tabs
		for (int i = 0; i < c.getBank().getBankTab().length; i++) {
			if (i == 0)
				continue;
			if (i != c.getBank().getBankTab().length - 1
					&& c.getBank().getBankTab()[i].size() == 0
		...
		c.getPA().sendFrame36(1667, 0);

		for (int index = 0; index < Bank.ITEM_CONTAINERS.length; index++) {
			sendItemContainer(Bank.ITEM_CONTAINERS[index], c.getBank().getBankTab()[index].getItems(), false);
		}
	}
	...
	public void resetTempItems() {
		...
		c.getOutStream().createFrameVarSizeWord(53);
		c.getOutStream().writeUnsignedWord(5064);
	...
	public void removeFromBank(int itemId, int itemAmount, boolean updateView) {
		removeFromBank(c.getBank().getCurrentBankTab(), itemId, itemAmount, updateView);
	}
	...
	public void swapBankItem(int from, int to) {
		swapBankItem(c.getBank().getCurrentBankTab(), from, to);
	}
	...
	public void moveItems(int from, int to, int moveWindow, boolean insertMode) {
		if (c.getBank().moveItems(from, to, moveWindow, insertMode)) {
			return;
		}
		if (moveWindow == 3214) {
			int tempI;
	...
	public void deleteItem(int id, int slot, int amount) {
		...
		if (c.playerItems[slot] == (id + 1)) {
			...
			PlayerSave.saveGame(c); /// TODO remove this
			this.addContainerUpdate(ContainerUpdate.INVENTORY);
		}
	}
	...
	public void dropArrowNpc(NPC npc) {
		...
		if (Misc.random(10) >= 4) {
			if (Server.itemHandler.itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height, npc.getInstance()) == 0) {
				Server.itemHandler.createGroundItem(c, c.rangeItemUsed, enemyX, enemyY, height, 1, c.getIndex());
			} else if (Server.itemHandler.itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height, npc.getInstance()) != 0) {
				int amount = Server.itemHandler.itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height, npc.getInstance());
	...
	public void dropArrowPlayer() {
		...
		if (c.playerEquipment[c.playerCape] == 10499 || c.getItems().isWearingItem(22109, c.playerCape) ||SkillcapePerks.RANGING.isWearing(c) || SkillcapePerks.isWearingMaxCape(c))
			return;
		if (Misc.random(10) >= 4) {
			if (Server.itemHandler.itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height, c.getInstance()) == 0) {
				Server.itemHandler.createGroundItem(c, c.rangeItemUsed, enemyX, enemyY, height, 1, c.getIndex());
			} else if (Server.itemHandler.itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height, c.getInstance()) != 0) {
				int amount = Server.itemHandler.itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY, height, c.getInstance());
				Server.itemHandler.removeGroundItem(c, c.rangeItemUsed, enemyX, enemyY, height, false);
				Server.itemHandler.createGroundItem(c, c.rangeItemUsed, enemyX, enemyY, height, amount + 1, c.getIndex());
			}
		}
	...
	public void removeGroundItem(GroundItem item) {
		...
		c.getOutStream().createFrame(156);
		c.getOutStream().writeByteS(0);
		c.getOutStream().writeUnsignedWord(item.getId());
		c.flushOutStream();
	}
