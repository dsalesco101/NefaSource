...
public class PathFinder {
	...
	public void findRoute(Player c, int destX, int destY, boolean moveNear,
		...
		while (tail != tileQueueX.size() && tileQueueX.size() < pathLength) {
			...
			if (curY > 0
					&& via[curX][curY - 1] == 0
					&& (c.getRegionProvider().getClipping(curAbsX, curAbsY - 1, c.heightLevel) & 0x1280102) == 0) {
				tileQueueX.add(curX);
			...
			if (curX > 0
					&& via[curX - 1][curY] == 0
					&& (c.getRegionProvider().getClipping(curAbsX - 1, curAbsY, c.heightLevel) & 0x1280108) == 0) {
			...
			if (curY < 104 - 1
					&& via[curX][curY + 1] == 0
					&& (c.getRegionProvider().getClipping(curAbsX, curAbsY + 1, c.heightLevel) & 0x1280120) == 0) {
				tileQueueX.add(curX);
				tileQueueY.add(curY + 1);
				via[curX][curY + 1] = 4;
				cost[curX][curY + 1] = thisCost;
			...
			if (curX < 104 - 1
					&& via[curX + 1][curY] == 0
					&& (c.getRegionProvider().getClipping(curAbsX + 1, curAbsY, c.heightLevel) & 0x1280180) == 0) {
				tileQueueX.add(curX + 1);
				tileQueueY.add(curY);
				via[curX + 1][curY] = 8;
				cost[curX + 1][curY] = thisCost;
			...
			if (curX > 0
					&& curY > 0
					&& via[curX - 1][curY - 1] == 0
					&& (c.getRegionProvider().getClipping(curAbsX - 1, curAbsY - 1,
