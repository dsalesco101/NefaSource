...
import static ethos.model.players.skills.slayer.SlayerRewardsInterfaceData.*;


public class SlayerRewardsInterface {

    public static void open(Player player, Tab tab) {
        switch (tab) {
            case UNLOCK:
                Arrays.stream(Unlock.values()).forEach(unlock ->
                        player.getPA().sendConfig(unlock.getConfig(), player.getSlayer().getUnlocks().contains(unlock.getUnlock()) ? 1 : 0));
                break;
            case EXTEND:
                Arrays.stream(Extend.values()).forEach(extend ->
            ...
            case TASK:
                // Blocked tasks
                for (int index = 0; index < BLOCKED_TASK_STRINGS.length; index++) {
                    ...
                    player.getPA().sendString(blocked, BLOCKED_TASK_STRINGS[index]);
                }
                ...
                player.getPA().sendString(task, CURRENT_ASSIGNMENT_STRING);
                break;
        }

        player.getPA().sendString(Misc.insertCommas(String.valueOf(player.getSlayer().getPoints())), CURRENT_SLAYER_POINTS_STRING);
    ...
    public static boolean clickButton(Player player, int buttonId) {
        ...
        for (Unlock unlock : Unlock.values()) {
            if (buttonId == unlock.getButton()) {
                if (!player.getSlayer().getUnlocks().contains(unlock.getUnlock())) {
                    ...
                    info(player, accept, plr -> open(plr, Tab.UNLOCK), unlock.getInformation());
                } else {
        ...
        for (Extend extend : Extend.values()) {
            if (buttonId == extend.getButton()) {
                if (!player.getSlayer().getExtensions().contains(extend.getUnlock())) {
                    ...
                    info(player, accept, plr -> open(plr, Tab.EXTEND), extend.getInformation());
                } else {
        ...
        // Unblock
        for (int index = 0; index < UNBLOCK_TASK_BUTTONS.length; index++) {
            final int indexId = index;
            if (buttonId == UNBLOCK_TASK_BUTTONS[index]) {
                String name = player.getSlayer().getRemoved()[indexId];
                if (name == null || name.length() == 0) {
                    player.sendMessage("You don't have a blocked task in this slot.");
                } else {
                    info(player, plr -> unblock(player, indexId), plr -> open(player, Tab.TASK),
        ...
        // Other buttons
        switch (buttonId) {
            case BLOCK_TASK_BUTTON:
                if (!player.getSlayer().getTask().isPresent()) {
                    player.sendMessage("You don't have a Slayer task.");
                } else {
                    info(player, SlayerRewardsInterface::block, plr -> open(player, Tab.TASK),
            ...
            case CANCEL_TASK_BUTTON:
                if (!player.getSlayer().getTask().isPresent()) {
                    ...
                    info(player, SlayerRewardsInterface::cancel, plr -> open(player, Tab.TASK),
                            "<ul>" + player.getSlayer().getTask().get().getFormattedName() + "</ul>",
                            "",
                ...
                return true;
            case INFO_CONFIRM_BUTTON:
                Object consumer = player.getAttributes().get(INFO_BOX_ACCEPT_CONSUMER_KEY);
                if (consumer != null) {
                    ((Consumer<Player>) consumer).accept(player);
                }
                return true;
            case INFO_DECLINE_BUTTON:
                consumer = player.getAttributes().get(INFO_BOX_DECLINE_CONSUMER_KEY);
                if (consumer != null) {
                    ((Consumer<Player>) consumer).accept(player);
                }
                return true;
    ...
    private static void info(Player player, Consumer<Player> accept, Consumer<Player> decline, String...strings) {
        Preconditions.checkArgument(strings.length <= INFO_BOX_STRINGS.length, "Too many strings!");
        player.getAttributes().set(INFO_BOX_ACCEPT_CONSUMER_KEY, accept);
        player.getAttributes().set(INFO_BOX_DECLINE_CONSUMER_KEY, decline);
        for (int index = 0; index < INFO_BOX_STRINGS.length; index++) {
            if (index < strings.length) {
                player.getPA().sendString(strings[index], INFO_BOX_STRINGS[index]);
            } else {
                player.getPA().sendString("", INFO_BOX_STRINGS[index]);
            }
        }
        player.getPA().showInterface(INFO_BOX_INTERFACE_ID);
    }
}
