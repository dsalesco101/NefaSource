 ...
 */
		...
		ItemDefinition definition = ItemDefinition.forId(item.getId());
		 if (Misc.random(stall.petChance) == 20 && player.getItems().getItemCount(20663, false) == 0 && player.summonId != 20663) {
			 PlayerHandler.executeGlobalMessage("[<col=CC0000>News</col>] <col=255>" + player.playerName + "</col> now goes hand in hand with a <col=CC0000>Rocky</col> pet!");
	...
	public enum Pickpocket {
		...
		}), FARMER(60, 80, 52000, new HashMap<Rarity, List<GameItem>>() {
			{
	...
	public enum Stall {
		...
		Silk(new GameItem(950), 50, 54, 10, 40000),
		Spice(new GameItem(2007), 50, 54, 10, 40000),
		Gold(new GameItem(4692), 95, 110, 10, 36500),
		LZ_GOLD(new GameItem(19473), 99, 120, 10, 36500);
