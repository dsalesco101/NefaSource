...
public class DialogueHandler {
	...
	public void sendDialogues(int dialogue, int npcId) {
		...
		case LootingBag.OPTIONS_DIALOGUE_ID:
			...
			break;
		case 564:
			sendNpcChat("Would you like to purchase pet insurance?");
			c.nextChat = 565;
			break;
		case 565:
			sendOption2("Yes", "No");
			c.dialogueAction = 565;
			break;
		case 353:
		...
		case 793:
			...
			break;
		case 380:
			sendNpcChat ("This cave leads to the Crystalline Hunllef Boss.", "Are you sure you want to enter now?", "You will lose your key upon entering");
			c.nextChat = 381;
			break;
		case 381:
			sendOption2("Yes, I am ready to fight.", "No, I'm scared.");
		...
		case 265:
			...
			break;
		case 361:
			sendNpcChat ("Would you like to exchange this item for 500 pvm points.");
			c.nextChat = 362;
			break;
		case 362:
			sendOption2("Yes", "No");
			...
			if (task.isPresent() && master.isPresent() && myMaster.isPresent()) {
				if (c.getSlayer().getMaster() != master.get().getId()) {
					...
					if (c.talkingNpc == 401) {
						...
						if (c.slayerPoints > 99) {
						sendNpcChat(
								"You currently have " + c.getSlayer().getTaskAmount() + " "
										+ c.getSlayer().getTask().get().getPrimaryName() + " to kill.",
								"Talk to Nieve for an easier task. If you obtain an",
								"easier task your consecutive tasks will be reset.");
						c.nextChat = -1;}
						else {
							sendNpcChat("You need to have a collection of 100 slayer points",
									"before you can get a task from me.");
						}
					}
				}
			} else {
		...
		case 644:
			Pair<String, Long> pair = Zulrah.getBestZulrahTime();
			sendNpcChat("The current record is set by @red@" + pair.getLeft() + "@bla@.",
					"The time to beat is @red@" + Misc.toFormattedMS(pair.getRight()) + "@bla@.");
			c.nextChat = 638;
			break;
		...
		case 9649:
			...
			break;
		case 650:
			if (c.getMode().isIronman() || c.getMode().isUltimateIronman() || c.getMode().isHCIronman() || c.getMode().isOsrs()) {
		...
		case 652:
			...
			if (c.getRevertOption().equals(Action.NPC.name()) || c.getMode().isOsrs()) {
				if (c.getRevertModeDelay() > 0) {
					...
					sendNpcChat("I can do this for you, but be aware",
		...
		case 655:
			...
			c.nextChat = -1;
			if (c.totalLevel > 750) {
				PlayerHandler.executeGlobalMessage("[@blu@REVERT@bla@]@red@ " + c.playerName + " @bla@has just reverted to a normie account!");
				}
			break;
		...
		case 661:
			...
			EquipmentSet set = c.getMode().isIronman() ? EquipmentSet.IRON_MAN_ARMOUR
					...
					: c.getMode().isHCIronman() ? EquipmentSet.HC_MAN_ARMOUR :
						null;
