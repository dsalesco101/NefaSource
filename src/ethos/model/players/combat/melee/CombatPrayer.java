...
public class CombatPrayer {
	...
	public static void activatePrayer(Player c, int i) {
		if (i == 18) {
            if ((TourneyManager.getSingleton().isInArena(c) || TourneyManager.getSingleton().isInLobbyBounds(c)) && TourneyManager.getSingleton().getCurrentPrayerBlock().equals("PROTECT_MELEE")) {
                c.getPA().sendFrame36(c.PRAYER_GLOW[i], 0);
		...
		if (i == 16) {
			if ((TourneyManager.getSingleton().isInArena(c) || TourneyManager.getSingleton().isInLobbyBounds(c)) && TourneyManager.getSingleton().getCurrentPrayerBlock().equals("PROTECT_MAGIC")) {
                c.getPA().sendFrame36(c.PRAYER_GLOW[i], 0);
                c.sendMessage("You cannot activate this prayer while in the arena");
                return;
            }
		...
		if (i == 17) {
			if ((TourneyManager.getSingleton().isInArena(c) || TourneyManager.getSingleton().isInLobbyBounds(c)) && TourneyManager.getSingleton().getCurrentPrayerBlock().equals("PROTECT_RANGE")) {
                c.getPA().sendFrame36(c.PRAYER_GLOW[i], 0);
                c.sendMessage("You cannot activate this prayer while in the arena");
                return;
            }
		...
		if (i == 25) {
			if (c.playerLevel[1] < 65) {
				if (c.playerLevel[5] < 60) {
					c.getPA().sendFrame36(c.PRAYER_GLOW[i], 0);
					c.getPA().sendFrame126("You need a @bl2@Prayer @bla@level of " + c.PRAYER_LEVEL_REQUIRED[i] + "and 60 defence to use @bl2@" + c.PRAYER_NAME[i] + "@bla@.", 357);
		...
		if (i == 26) {
			if (c.playerLevel[1] < 70) {
				if (c.playerLevel[5] < 70) {
					c.getPA().sendFrame36(c.PRAYER_GLOW[i], 0);
					c.getPA().sendFrame126("You need a @bl2@Prayer @bla@level of " + c.PRAYER_LEVEL_REQUIRED[i] + " and 70 defence to use @bl2@" + c.PRAYER_NAME[i] + "@bla@.", 357);
					c.getPA().sendFrame126("Click here to continue", 358);
					c.getPA().sendFrame164(356);
				}
				c.nextChat = -1;
