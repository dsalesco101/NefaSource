...
public class MeleeMaxHit {
	...
	public static double calculateBaseDamage(Player c, boolean special) {
		...
		if (c.npcAttackingIndex > 0) {
			...
			if (npc != null) {
				if (c.getSlayer().getTask().isPresent()) {
					if (npc != null && c.getSlayer().getTask().get().matches(npc.getDefinition().getNpcName())) {
				...
				if (c.getItems().isWearingItem(4081, c.playerAmulet)) {
					if (Misc.linearSearch(Config.UNDEAD_NPCS, npc.npcType) != -1) {
						base *= 1.15;
					}
				}
				if (c.getItems().isWearingItem(22978)) {
		            NPC npc1 = NPCHandler.npcs[c.npcAttackingIndex];
				  if (Misc.linearSearch(Config.DRAG_IDS, npc.npcType) != -1) {
			            base *= 1.60;
			        }
				}
				...
				if (c.getItems().isWearingItem(10588, c.playerAmulet)) {
					if (Misc.linearSearch(Config.UNDEAD_NPCS, npc.npcType) != -1) {
						base *= 1.20;
					}
				}
		...
		if (c.npcAttackingIndex > 0 && c.inWild() && c.playerEquipment[c.playerWeapon] == 22545 ) { //viggora chainmace
			base *= 1.5; //50%
			}
	...
	public static final double[][] special = { { 5698, 1.05 }, { 5680, 1.05 }, { 1231, 1.05 }, { 1215, 1.05 }, { 3204, 0.90 }, { 1305, 1.15 }, { 1434, 1.45 }, { 11802, 1.375 },
			{ 20368, 1.375 }, { 22622, 1.375 }, { 20370, 1.21 }, { 20374, 1.10 }, { 20372, 1.10 },
			{ 11804, 1.21 }, { 19675, 1.10 }, { 11806, 1.10 }, { 11808, 1.10 }, { 861, 1.1 }, { 4151, 1.1 }, { 10887, 1.2933 }, { 12926, 1.35 } };
	...
	public static int calculateMeleeAttack(Player c) {
		...
		NPC npc1 = NPCHandler.npcs[c.npcAttackingIndex];
		if (c.playerEquipment[c.playerShield] == 4081 && Misc.linearSearch(Config.UNDEAD_NPCS, npc1.npcType) != -1) { //salve amulet
			attackLevel += c.getLevelForXP(c.playerXP[c.playerAttack]) * 0.15;
        }
        if (c.playerEquipment[c.playerShield] == 10588 && Misc.linearSearch(Config.UNDEAD_NPCS, npc1.npcType) != -1) { //salve amulet (e)
