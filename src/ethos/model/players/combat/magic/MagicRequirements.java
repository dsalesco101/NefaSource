...
public class MagicRequirements extends MagicConfig {
	...
	public static boolean checkMagicReqs(Player c, int spell) {
		...
		if (c.usingMagic && c.playerAttackingIndex > 0) {
			if (PlayerHandler.players[c.playerAttackingIndex] != null) {
				for (int r = 0; r < c.REDUCE_SPELLS.length; r++) { //Reducing/weaken spells
					if (PlayerHandler.players[c.playerAttackingIndex].REDUCE_SPELLS[r] == MagicData.MAGIC_SPELLS[spell][0]) {
						c.reduceSpellId = r;
						if ((System.currentTimeMillis() - PlayerHandler.players[c.playerAttackingIndex].reduceSpellDelay[c.reduceSpellId]) > PlayerHandler.players[c.playerAttackingIndex].REDUCE_SPELL_TIME[c.reduceSpellId]) {
							PlayerHandler.players[c.playerAttackingIndex].canUseReducingSpell[c.reduceSpellId] = true;
						} else {
							PlayerHandler.players[c.playerAttackingIndex].canUseReducingSpell[c.reduceSpellId] = false;
						}
						break;
					}
				}
				if (!PlayerHandler.players[c.playerAttackingIndex].canUseReducingSpell[c.reduceSpellId]) {
