...
public class MagicMaxHit {
	...
	public static int mageAttack(Player c) {
		...
		if (c.getItems().isWearingItem(20714, c.playerShield) && !(c.spellId == 1539) || !(c.spellId == 1192)
		...
		if (c.npcAttackingIndex > 0 && c.getSlayer().getTask().isPresent()) {
			...
			if ((npc.npcType > 1) && c.inWild() && c.playerEquipment[c.playerWeapon] == 22555) {
					modifier *= 2;
			}
		}
	...
	public static int magiMaxHit(Player c) {
		...
		if (c.npcAttackingIndex > 0 && c.inWild() && c.playerEquipment[c.playerWeapon] == 22555) { //thammaron sceptre
			damageMultiplier *= 1.25; //50%
			}
		...
		case 12037:
			if (c.getItems().isWearingAnyItem(21255) && c.getSlayer().getTask().isPresent()) {			
