...
import ethos.model.players.combat.formula.MagicSuccessFormula;
...
public class AttackPlayer {
	...
	public static final int STANDARD_IN_COMBAT_DELAY = 10_000;

	public static final int KILLED_PLAYER_COMBAT_DELAY = 4_000;
	...
	public static void applyPlayerMeleeDamage(Player c, int i, int damageMask, int damage, Hitmark hitmark) {
		...
		if (PlayerHandler.players[i].getHealth().getCurrentHealth() - damage < 0) {
			damage = PlayerHandler.players[i].getHealth().getCurrentHealth();
		}
		...
		o.underAttackByPlayer = c.getIndex();
	...
	public static void addCombatXP(Player c, CombatType type, int damage) {
		boolean showXpDrops = !(c.playerAttackingIndex > 0);
		int experience = 0;
		...
		case MELEE:
			...
			c.getPA().addSkillXP((int) Math.ceil((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE))) / 3, 3, showXpDrops);
			...
			case 0: // Accurate
				...
				c.getPA().addSkillXP((int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE)), 0, showXpDrops);
				break;
			case 1: // Block
				skills.add(Skill.DEFENCE.getId());
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE));
				c.getPA().addSkillXP((int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE)), 1, showXpDrops);
				break;
			case 2: // Aggressive
				skills.add(Skill.STRENGTH.getId());
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE));
				c.getPA().addSkillXP((int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE)), 2, showXpDrops);
				break;
			case 3: // Controlled
				int split = (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE)) / 3;
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE));
				for (int i = 0; i < 3; i++) {
					c.getPA().addSkillXP(split, i, showXpDrops);
					skills.add(i);
				}
				break;
			}
		...
		case RANGE:
			...
			c.getPA().addSkillXP((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE)) / 3, 3, showXpDrops);
			switch (c.fightMode) {
			case 0: // Accurate
			case 2: // Rapid
				skills.add(Skill.RANGED.getId());
				experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE));
				c.getPA().addSkillXP(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE), 4, showXpDrops);
				break;
			case 1: // Block
			case 3:
				skills.add(Skill.RANGED.getId());
		...
		case MAGE:
			...
			experience += ((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MAGIC_EXP_RATE)) / 3)
					+ (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MAGIC_EXP_RATE));

			c.getPA().addSkillXP((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MAGIC_EXP_RATE)) / 3, 3, showXpDrops);

			int combatXp = damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MAGIC_EXP_RATE);
			if (c.autocastingDefensive) {
				c.getPA().addSkillXP(combatXp / 2, 6, showXpDrops);
				c.getPA().addSkillXP(combatXp / 2, 1, showXpDrops);
			} else {
				c.getPA().addSkillXP(combatXp, 6, showXpDrops);
			}
			break;
		}
		if (experience > 0) {
	...
	public static void playerDelayedHit(final Player c, final int i, final Damage d) {
		if (PlayerHandler.players[i] != null) {
			Player o = PlayerHandler.players[i];
			if (o == null || o.isDead || c.isDead || PlayerHandler.players[i].getHealth().getCurrentHealth() <= 0 || c.getHealth().getCurrentHealth() <= 0) {
			...
			if (o.playerAttackingIndex <= 0 && o.npcAttackingIndex <= 0) {
				if (o.autoRet == 1) {
					o.combatFollowing = true;
					o.playerAttackingIndex = c.getIndex();
			...
			if (o.attackTimer <= 3 || o.attackTimer == 0 && o.playerAttackingIndex == 0 && !c.usingMagic) { // block
																									// animation
			...
			if (!c.usingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (damage > PlayerHandler.players[i].getHealth().getCurrentHealth()) {
					damage = PlayerHandler.players[i].getHealth().getCurrentHealth();
				}
				o.underAttackByPlayer = c.getIndex();
				o.logoutDelay = System.currentTimeMillis();
				...
				if (!c.magicFailed) {
					if (System.currentTimeMillis() - PlayerHandler.players[i].reduceStat > 35000) {
						...
						case 13023:
							...
							PlayerHandler.players[i].getSkills().sendRefresh();
							break;
						}
					}
	...
	public static void calculateCombatDamage(Player attacker, Player defender, CombatType combatType, Special special) {
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
                ...
                attacker.getPA().addXpDrop(new PlayerAssistant.XpDrop((damage > 0 ? damage : 0) + (damage2 > 0 ? damage2 : 0) + (damage3 > 0 ? damage3 : 0), Skill.ATTACK.getId()));
				...
				attacker.getPA().addXpDrop(new PlayerAssistant.XpDrop((damage > 0 ? damage : 0) + (damage2 > 0 ? damage2 : 0) + (damage3 > 0 ? damage3 : 0), Skill.RANGED.getId()));
				...
				if (!MagicSuccessFormula.isHit(attacker, defender)) {
					damage = 0;
				...
				if (freezeDelay > 0 && defender.freezeTimer <= -3 && !attacker.magicFailed) {
					...
					defender.getPA().sendGameTimer(ClientGameTimer.FREEZE, TimeUnit.MILLISECONDS, 600 * freezeDelay);
				...
				attacker.getPA().addXpDrop(new PlayerAssistant.XpDrop((damage > 0 ? damage : 0) + (damage2 > 0 ? damage2 : 0) + (damage3 > 0 ? damage3 : 0), Skill.MAGIC.getId()));
			}
		}
	...
	public static void attackPlayer(Player c, int i) {
		...
		if (c.checkCombatDistance(c, o, false)) {
			resetSpells(c);
			if (c.playerAttackingIndex >= PlayerHandler.players.length || c.playerAttackingIndex < 0) {
				return;
			}
			if (PlayerHandler.players[i] != null) {
				...
				if (c.connectedFrom.equals(c2.connectedFrom) && !c.getRights().isOrInherits(Right.MODERATOR) && Server.getConfiguration().getServerState().getPort() != 5555 && !Boundary.isIn(c, Boundary.OUTLAST)) {
				...
				if (c.getItems().isWearingAnyItem(4212, 4213, 4214, 4215, 4216, 4217, 4218, 4219, 4220, 4221, 4222, 4223, 22550) && c.playerEquipment[c.playerArrows] > -1) {
					c.sendMessage("You cannot use any arrows with this bow.");
					c.getCombat().resetPlayerAttack();
					return;
				}
				if (c.getItems().isWearingAnyItem() && c.playerEquipment[c.playerArrows] > -1) {
					c.sendMessage("You cannot use any arrows with this bow.");
					c.getCombat().resetPlayerAttack();
					return;
				}
				...
				if (PlayerHandler.players[i].respawnTimer > 0) {
					PlayerHandler.players[i].playerAttackingIndex = 0;
					c.getCombat().resetPlayerAttack();
					return;
				}
				...
				if (c.attackTimer <= 0) {
					...
					if (!c.usingBallista && !c.usingCross && !c.usingArrows && c.usingBow && !(c.playerEquipment[c.playerWeapon] == 22550) 
					...
					if (!c.getCombat().correctBowAndArrows() && Config.CORRECT_ARROWS && c.usingBow && !c.getCombat().usingCrystalBow() &&
							!(c.playerEquipment[c.playerWeapon] == 22550) && c.playerEquipment[c.playerWeapon] != 4734
							&& c.playerEquipment[c.playerWeapon] != 9185 && c.playerEquipment[c.playerWeapon] != 11785 && c.playerEquipment[c.playerWeapon] != 21012 && !c.usingMagic && !c.getItems().isWearingItem(12926) && c.playerEquipment[c.playerWeapon] != 19478 && c.playerEquipment[c.playerWeapon] != 19481 && c.playerEquipment[c.playerWeapon] != 21902) {
					...
					if (c.playerEquipment[c.playerWeapon] == 9185 && !c.getCombat().properBolts() && !c.usingMagic || c.playerEquipment[c.playerWeapon] == 21902 && !c.getCombat().properBolts() && !c.usingMagic
					...
					boolean projectile = c.usingOtherRangeWeapons || c.usingBow || c.usingMagic || c.getCombat().usingCrystalBow() ||
							c.playerEquipment[c.playerWeapon] == 22550 || c.usingMelee;
					
					if (projectile) {
						if (!PathChecker.isProjectilePathClear(c, o, c.absX, c.absY, c.heightLevel, o.absX, o.absY) 
								&& !PathChecker.isProjectilePathClear(c, o, o.absX, o.absY, c.heightLevel, c.absX, c.absY)) {
							c.attackTimer = 1;
							return;
						} else {
							//c.stopMovement();
					...
					// attack successful begins here

					// Reset the target's underAttackBy so they can attack back
					if (c.getBH().isTarget(c.playerAttackingIndex)) {
						Player other = PlayerHandler.players[c.playerAttackingIndex];
						other.underAttackByNpc = -1;
						other.underAttackByPlayer = -1;
					}
					...
					if (c.usingSpecial && !c.usingMagic) {
						...
						c.followId = c.playerAttackingIndex;
						c.oldPlayerIndex = c.playerAttackingIndex;
					...
					if (c.getHealth().getCurrentHealth() > 0 && !c.isDead && PlayerHandler.players[i].getHealth().getCurrentHealth() > 0) {
						if (!c.usingMagic) {
					...
					target.underAttackByPlayer = c.getIndex();
					...
					if (!c.usingBow && !c.usingMagic && !c.usingSpecial && !c.usingOtherRangeWeapons) { // melee
						c.followId = PlayerHandler.players[c.playerAttackingIndex].getIndex();
					...
					if (c.usingMagic) { // magic hit delay
						...
						if (c.playerEquipment[c.playerShield] == 20714 && c.pages <= 0) { // tome of fire	
							c.sendMessage("Your tome of fire has run out of pages.");
							c.getCombat().resetPlayerAttack();
							return;
						}
						if (c.playerEquipment[c.playerWeapon] == 22550 && c.crawsbowCharge <= 1000) { // new		
							c.sendMessage("Your weapon has ran out of ether and can no longer function.");
							c.getCombat().resetPlayerAttack();
							return;
						}
						if (c.playerEquipment[c.playerWeapon] == 22545 && c.viggoraCharge <= 1000) { // new		
								c.sendMessage("Your weapon has ran out of ether and can no longer function."); 
								c.getCombat().resetPlayerAttack();
								return;
				             }
						if (c.playerEquipment[c.playerWeapon] == 22555 && c.thammaronCharge <= 1000) { // new		
								c.sendMessage("Your weapon has ran out of ether and can no longer function."); 
								c.getCombat().resetPlayerAttack();
								return;
						}
						...
						if (c.playerEquipment[c.playerWeapon] == 22555 && c.thammaronCharge <= 0) { // new								
						 ...
						 if (c.playerEquipment[c.playerShield] == 20714 && !(c.spellId == 1181) || !(c.spellId == 1169) || !(c.spellId == 1158)
								 || !(c.spellId == 1539) || !(c.spellId == 1192)
									|| !(c.spellId == 12939) || !(c.spellId == 12963) || !(c.spellId == 12951) || !(c.spellId == 12975)
									|| !(c.spellId == 12975)) { //tome of fire
								c.decreasePages(1);
							 }
						if (c.autocastId > 0) {
							c.followId = c.playerAttackingIndex;
					...
					// Reset magic attack if not autocasting
					if (c.usingMagic && !c.autocasting && c.spellId <= 0)
						c.playerAttackingIndex = 0;
					...
					if (c.usingBow && Config.CRYSTAL_BOW_DEGRADES) { // crystal
						...
						if (c.crawsbowCharge <= 0 &&c.playerEquipment[c.playerWeapon] == 22550) { // new
							c.getItems().wearItem(22547, 1, 3);
						}
