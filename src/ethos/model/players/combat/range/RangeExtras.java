...
public class RangeExtras {
	...
	public static boolean checkMultiChinchompaReqsNPC(int i) {
		...
		if (NPCHandler.npcs[i].npcType == 6611 || NPCHandler.npcs[i].npcType == 6612) {
			...
			if (minion.stream().filter(Objects::nonNull)
					.anyMatch(n -> n.npcType == 5054 && !n.isDead && n.getHealth().getCurrentHealth() > 0)) {
				return false;
			}
		}
		return true;
	...
	public static boolean checkMultiChinReqs(Player c, int i) {
		...
		if (Config.SINGLE_AND_MULTI_ZONES) {
			if (!PlayerHandler.players[i].inMulti()) { // single combat zones
				if (PlayerHandler.players[i].underAttackByPlayer != c.getIndex() && PlayerHandler.players[i].underAttackByPlayer != 0) {
					return false;
				}
				if (PlayerHandler.players[i].getIndex() != c.underAttackByPlayer && c.underAttackByPlayer != 0) {
		...
		if (NPCHandler.npcs[npcId] != null) {
			...
			if (checkMultiChinchompaReqsNPC(npcId)) {
				...
				if (Misc.random(c.getCombat().calculateRangeAttack()) > Misc.random(NPCHandler.npcs[npcId].defence)) {
					...
					if (n.getHealth().getCurrentHealth() - damage < 0) {
						damage = n.getHealth().getCurrentHealth();
					}
					if (damage == 0) {
						return;
					}
	...
	public static boolean wearingCrossbow(Player player) {
		return player.playerEquipment[player.playerWeapon] == 11785 || player.playerEquipment[player.playerWeapon] == 9185 || player.playerEquipment[player.playerWeapon] == 21012 || player.playerEquipment[player.playerWeapon] == 21902;
	}
