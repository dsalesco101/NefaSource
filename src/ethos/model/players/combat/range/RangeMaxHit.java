...
public class RangeMaxHit extends RangeData {
	...
	public static int calculateRangeAttack(Player c) {
		...
		if (c.npcAttackingIndex > 0 && c.getItems().isWearingItem(20997)) {
			rangeLevel += 1.2;
		}
		if (c.fullVoidRange()) {
	...
	public static int maxHit(Player c) {
		...
		int rangedStrength = 
			 ...
			 */
			c.playerEquipment[c.playerWeapon] != 12926 && !wearingCrystalBow(c) && c.playerEquipment[c.playerWeapon] != 22550 && c.usingBow || c.usingBallista ? getRangeStr(c.playerEquipment[c.playerArrows]) :
			 ...
			 */
			wearingCrystalBow(c) || c.playerEquipment[c.playerWeapon] == 22550 ? getRangeStr(c.playerEquipment[c.playerWeapon]) :
		...
		if (c.npcAttackingIndex > 0 && c.inWild() && c.playerEquipment[c.playerWeapon] == 22550 ) { //craw bow
			b *= 1.5; //50%
			}
		if (c.npcAttackingIndex > 0 && c.getItems().isWearingItem(20997)) {
			b*= 2.65;
