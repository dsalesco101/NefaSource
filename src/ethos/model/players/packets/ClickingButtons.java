...
import ethos.model.players.packets.dialogueoptions.OptionHandler;
...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		if (c.getCollectionLog().handleActionButtons(actionButtonId)) {
			return;
		}
		...
		if (c.getTutorial().isActive()) {
			if (actionButtonId == 165179) {
				if (System.currentTimeMillis() - c.potDelay > 6000) {
		...
		case 132139://dark beast
			...
			break;
		case 185152:
			c.getCollectionLog().openInterface();
			break;
		case 19236:
		...
		case 57057:
			...
			break;
		case 183156:
			if (c.inDonatorBox == false && !c.getUltraInterface().isActive() && !c.getSuperBoxInterface().isActive() && !c.getNormalBoxInterface().isActive()) {
		...
		case 113249:
			...
			// c.checkWellOfGoodwillTimers(); fuck hmm
			break;
		...
		case 175202:
			...
			for (Player p : PlayerHandler.players) {
				if (p == null)
					continue;
				c.getPA().sendFrame126(
						"@or2@" + p.playerName + "  -  " + p.getHealth().getCurrentHealth() + "/" + p.getHealth().getMaximumHealth(),
						line);
				line++;
			}
			c.setSidebarInterface(2, 45000);
		...
		case 227018:
			...
			if (tab.size() + c.getBank().getBankTab()[0].size() >= Config.BANK_TAB_SIZE) {
				c.sendMessage("You cannot collapse this tab. The contents of this tab and your");
				c.sendMessage("main tab are greater than " + Config.BANK_TAB_SIZE + " unique items.");
				return;
			}
		...
		case 11015:
			...
			if (c.getItems().playerHasItem(13148)) {
				...
				c.playerLevel[c.antiqueSelect] = 1;
				c.playerXP[c.antiqueSelect] = c.getPA().getXPForLevel(1) + 1;
				c.getPA().refreshSkill(c.antiqueSelect);
				c.getPA().setSkillLevel(c.antiqueSelect, c.playerLevel[c.antiqueSelect], c.playerXP[c.antiqueSelect]);
				c.getItems().deleteItem(13148, 1);
				c.sendMessage("@red@You have reset your skill of choice.");
				c.getPA().closeAllWindows();
				return;
			}
			if (!c.getItems().playerHasItem(2528) && (!c.getItems().playerHasItem(21027) && (!c.getItems().playerHasItem(13148)))) {
				return;
			}
			if (c.usingLamp) {
				if (c.getItems().playerHasItem(2528) && c.normalLamp && !c.antiqueLamp && c.getMode().isRegular() || c.getMode().isHCIronman() || c.getMode().isUltimateIronman() || c.getMode().isIronman()) {
					c.usingLamp = false;
					c.inLamp = false;
					c.getPA().addSkillXP(125000, c.antiqueSelect, true); //note: thie experience doubles in game, so 125 = 250
					c.getItems().deleteItem(2528, 1);
					c.sendMessage("The lamp mysteriously vanishes...");
					c.sendMessage("...and you gain some experience!");
					c.getPA().closeAllWindows();
				} else if (c.normalLamp && !c.antiqueLamp && c.getItems().playerHasItem(2528, 1) && c.getMode().isOsrs()) {
					c.usingLamp = false;
					c.inLamp = false;
		...
		case 1094:
		case 1097:
			if (c.autocastId > 0 && (actionButtonId == 94047 && c.autocastingDefensive || actionButtonId == 1093 && !c.autocastingDefensive)) {
				c.getPA().resetAutocast();
			} else {
				if (c.autocastId > 0 && !c.autocastingDefensive && actionButtonId == 1093
						|| c.autocastId > 0 && c.autocastingDefensive && actionButtonId == 349) {
					c.getPA().resetAutocast();
				}
				if (c.playerMagicBook == 1) {
					if (c.playerEquipment[c.playerWeapon] == 4675 || c.playerEquipment[c.playerWeapon] == 6914
							|| c.playerEquipment[c.playerWeapon] == 21006 || c.playerEquipment[c.playerWeapon] == 4710|| c.playerEquipment[c.playerWeapon] == 11907
					...
					if (c.playerEquipment[c.playerWeapon] == 4170 || c.playerEquipment[c.playerWeapon] == 21255) {
						c.autocastingDefensive = actionButtonId == 94047;
		...
		case 29113:
			...
			if (c.getItems().isWearingItem(11791, c.playerWeapon)
				...
				MeleeExtras.graniteMaulSpecial(c, true);
			} else {
				c.usingSpecial = !c.usingSpecial;
				c.getItems().updateSpecialBar();
			}
