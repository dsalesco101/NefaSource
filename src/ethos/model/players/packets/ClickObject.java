	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		case FIRST_CLICK:
			...
			case 31622:
				...
				if (c.usingPrayer == true) {
					c.sendMessage("@red@Please turn off your prayer.");
					return;
				}
				c.specRestore = 120;
				c.specAmount = 10.0;
			...
			case 6097:
				...
				c.getPA().sendFrame126("" + Misc.getValueWithoutRepresentation(Wogw.MONEY_TOWARDS_EXPERIENCE) + "/50M",
						38012);
				c.getPA().sendFrame126("" + Misc.getValueWithoutRepresentation(Wogw.MONEY_TOWARDS_PC_POINTS) + "/50M",
						38013);
				c.getPA().sendFrame126(
						"" + Misc.getValueWithoutRepresentation(Wogw.MONEY_TOWARDS_DOUBLE_DROPS) + "/100M", 38014);
				c.sendMessage("@red@Donate for Double xp/double pkp reward and more!");
				break;
			case 30283:
		...
		case SECOND_CLICK:
			...
			switch (c.objectId) {
			case 31622:
				c.sendMessage("There are " + TourneyManager.getSingleton().getPlayers() + " players in the tournament.");
				break;
		...
		case THIRD_CLICK:
			...
			switch (c.objectId) {
			case 31622:
				c.sendMessage(TourneyManager.getSingleton().isLobbyOpen() ? "The tournament lobby is open." : TourneyManager.getSingleton().isArenaActive() ? "The tournament is currently active." : "The tournament is unopen and inactive.");
				break;
			case 7811:
				c.objectDistance = 3;
				break;
