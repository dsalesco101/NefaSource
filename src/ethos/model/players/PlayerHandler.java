...
public class PlayerHandler {
	...
	private static Queue<Runnable> queuedActions = new ConcurrentLinkedQueue<>();
	...
	public void process() {
		...
		synchronized (lock) {
			...
			for (int i = 0; i < Config.MAX_PLAYERS; i++) {
				if (players[randomOrder[i]] == null || !players[randomOrder[i]].isActive || !players[randomOrder[i]].initialized)
					continue;
				Player c = players[randomOrder[i]];
				if (c.followId > 0) {
			...
			for (int i = 0; i < Config.MAX_PLAYERS; i++) {
				...
				try {
					...
					if (players[randomOrder[i]].disconnected && (System.currentTimeMillis() - players[randomOrder[i]].logoutDelay > 10000
							|| players[randomOrder[i]].disconnected && players[randomOrder[i]].disconnectTime != 0 &&
									(System.currentTimeMillis() - players[randomOrder[i]].disconnectTime >= 30_000)
							|| players[randomOrder[i]].properLogout || kickAllPlayers
			...
			for (int i = 0; i < Config.MAX_PLAYERS; i++) {
				if (players[randomOrder[i]] == null || !players[randomOrder[i]].isActive || !players[randomOrder[i]].initialized)
					continue;
				players[randomOrder[i]].processDamageQueue();
				players[randomOrder[i]].postProcessing();
				players[randomOrder[i]].getNextPlayerMovement();
				players[randomOrder[i]].checkInstanceCoords();
			}
	...
	public void updateNPC(Player plr, Stream str) {
		...
		for (int i = 0; i < size; i++) {
			if (plr.rebuildNPCList == false && plr.withinDistance(plr.npcList[i]) == true && !plr.npcList[i].teleporting && plr.npcList[i].getInstance() == plr.getInstance()) {
		...
		for (int i = 0; i < NPCHandler.maxNPCs; i++) {
			if (NPCHandler.npcs[i] != null) {
				...
				} else if (plr.withinDistance(NPCHandler.npcs[i]) == false || plr.getInstance() != NPCHandler.npcs[i].getInstance()) {
	...
	public void updatePlayer(Player plr, Stream str) {
		...
		for (int i = 0; i < size; i++) {
			if (!plr.didTeleport && !plr.playerList[i].didTeleport && plr.withinDistance(plr.playerList[i]) && plr.getInstance() == plr.playerList[i].getInstance()) {
