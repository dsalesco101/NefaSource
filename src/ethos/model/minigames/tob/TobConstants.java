...
import ethos.model.items.LootItem;
...
public class TobConstants {
	...
	/*import galax.model.entity.player.GlobalMessages;
	import galax.model.entity.player.Player;
	import galax.model.items.ItemDefinition;
	import galax.model.items.ItemRarity;
	import galax.model.items.LootItem;
	import galax.util.Misc;*/
		...
		public static void giveLoot(Player p) {
			...
			if (p.theatrePoints > 500) {
				if (Misc.random(1,100) > 96) {
					...
					PlayerHandler.executeGlobalMessage(p.playerName+" received a "+ItemDefinition.forId(rare.getId()).getName()+" from Theatre of Blood!");
				} else {
