...
public class XericRewards {
	...
	public static void rareDrop(Player player) {
		...
		PlayerHandler.executeGlobalMessage(player.playerName + " has received a " + ItemDefinition.forId(rareDropItem[rareitem]).getName()  + " from Trials of Xeric!");
	}
	...
	public static void commonDrop(Player player) {
		...
		if (drop == 1957) {
			qty = 1;
			PlayerHandler.executeGlobalMessage(player.playerName+" has received THE ONION as a reward from Trials of Xeric");
		} else {
