...
public class ShopAssistant {
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 80) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs [ @pur@" + getSpecialItemValue(removeId) + " @bla@] PKP Points.");
			return;
		}
		if (c.myShopId == 121) {
		...
		if (c.myShopId == 15) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs [ @pur@" + getSpecialItemValue(removeId) + " @bla@] Christmas Points.");
			return;
		}
		if (c.myShopId == 171) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs [ @pur@" + getSpecialItemValue(removeId) + " @bla@] Exchange Points.");
			return;
		}
		if (c.myShopId == 172 || c.myShopId == 173) {
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently exchanges for  [ @pur@" + getSpecialItemValue(removeId) + " @bla@] Exchange Points.");
			return;
		}
		if (c.myShopId == 131) {
	...
	public void handleOtherShop(int itemID, int amount) {
		...
		if (c.myShopId == 44) {
			...
			if (c.christmasP >= getSpecialItemValue(itemID) * amount) {
				if (c.getItems().freeSlots() > 0) {
					c.christmasP -= getSpecialItemValue(itemID) * amount;
					c.refreshQuestTab(1);
					c.getItems().addItem(itemID, amount);
					c.getItems().resetItems(3823);
					logShop("bought", itemID, amount);
			...
			if (c.exchangeP >= getSpecialItemValue(itemID) * amount) {
				if (c.getItems().freeSlots() > 0) {
					c.exchangeP -= getSpecialItemValue(itemID) * amount;
					c.refreshQuestTab(1);
					c.getItems().addItem(itemID, amount);
					c.getItems().resetItems(3823);
					logShop("bought", itemID, amount);
