	...
	public Entity calculateKiller() {
		...
		for (Entry<Entity, List<Damage>> entry : damageTaken.entrySet()) {
			...
			if (killer != null && killer instanceof Player && this instanceof NPC) {
				...
				if ((player.getMode().isIronman() || player.getMode().isUltimateIronman()  || player.getMode().isUltimateIronman())
					...
					double percentile = ((double) totalDamage / (double) npc.getHealth().getMaximumHealth()) * 100D;
					if (percentile < 75.0) {
						killer = null;
					}
				}
