...
public class DiceHandler {
	...
	public static void rollDice(Player c) {
		...
		if (System.currentTimeMillis() - c.diceDelay >= 3000) {
			...
			c.forcedChat("[DICE]" + Misc.capitalizeJustFirst(c.playerName) + " rolled " + roll + " on the percentile dice.");
			PlayerLogging.write(PlayerLogging.LogType.DICE, c, String.format("Rolled %s", roll));

			PlayerHandler.nonNullStream()
					.filter(player -> player.distanceToPoint(c.absX, c.absY) < 20 && player.heightLevel == 0)
					.forEach(player -> {
						player.sendMessage("[@red@DICE@bla@] @blu@" + Misc.capitalizeJustFirst(c.playerName)
