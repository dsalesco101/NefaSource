...
public class AchievementHandler {
     ...
     */
    public int getAmountCompletedForTier(int tier) {
        int count = 0;
        for(int i = 0; i < completed[tier].length; i++) {
            if (completed[tier][i]) {
                count++;
            }
        }
    ...
    public void drawInterface(int tier) {
        ...
        for (Achievement achievement : Achievement.ACHIEVEMENTS) {
            if (achievement.getTier().ordinal() == tier) {
                ...
                player.getPA().sendFrame126((isComplete(achievement.getTier().ordinal(), achievement.getId()) ? "@gre@" : "@red@") + Misc.optimizeText(achievement.name().toLowerCase().replace("_", " ")), count);
                count++;
            }
        }
