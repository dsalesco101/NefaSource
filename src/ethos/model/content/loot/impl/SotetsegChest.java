...
public class SotetsegChest implements Lootable {
    ...
    private static GameItem randomChestRewardsYoutube(int chance) {
        ...
        List<GameItem> itemList = random <= 980 ? items.get(LootRarity.COMMON) : items.get(LootRarity.RARE);
        return Misc.getRandomItem(itemList);
    }

    private static GameItem randomChestRewards(int chance) {
        int random = Misc.random(chance);
        List<GameItem> itemList = random <= 998 ? items.get(LootRarity.COMMON) : items.get(LootRarity.RARE);
        return Misc.getRandomItem(itemList);
    }
