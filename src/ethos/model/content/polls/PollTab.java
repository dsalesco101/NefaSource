package ethos.model.content.polls;
...
public class PollTab {
    ...
    public static void init() {
        try {
            Path path = Paths.get("./data/json/polls_backup.json");
    ...
    public static void reloadPoll() {
        try {
            Path path = Paths.get("./data/json/poll.json");
            File file = path.toFile();
    ...
    public static void updateInterface(Player player) {
        ...
        if (!poll.getVoters().contains(player.playerName)) {
            player.setSidebarInterface(13, 21406);
    ...
    public static void vote(Player player) {
        ...
        poll.getVoters().add(player.playerName);
        player.votePoints += 1;
    ...
    public static void savePolls() {
        ...
        BufferedWriter bw;
        try {
            bw = new BufferedWriter(new FileWriter(new File("./data/json/polls_backup.json")));
            bw.write(prettyJson);
            bw.flush();
            bw.close();
        } catch (Exception e) {
    ...
    public static boolean isPollActive(Poll poll) {
        ...
        if (poll.getStartTime() + milliseconds > System.currentTimeMillis()) {
            return true;
        } else {
            return false;
        }
