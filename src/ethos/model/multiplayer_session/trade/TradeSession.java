...
/*				PlayerLogging.write(LogType.TRADE_LOG, player,
	...
	public void give() {
		...
		for (Player player : items.keySet()) {
			...
			PlayerLogging.write(PlayerLogging.LogType.TRADE, getOther(player), String.format("Received from [%s] [%s]", player.playerName, items.get(player).toString()));
			PlayerLogging.write(PlayerLogging.LogType.TRADE, player, String.format("Gave to [%s] [%s]", getOther(player).playerName, items.get(player).toString()));
		}
	}
