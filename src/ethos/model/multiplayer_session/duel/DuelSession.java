	...
	public void give() {
		...
		if (!Objects.equals(getOther(winner.get()), winner.get())) {
			...
			if (items.get(winner.get()).size() > 0) {
				...
				PlayerLogging.write(PlayerLogging.LogType.STAKE, winner.get(), String.format("Received from [%s] [%s]", other.playerName, items.get(winner.get()).toString()));
				PlayerLogging.write(PlayerLogging.LogType.STAKE, other, String.format("Lost to [%s] [%s]", winner.get().playerName, items.get(winner.get()).toString()));
			}
