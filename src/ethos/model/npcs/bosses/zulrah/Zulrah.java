...
public class Zulrah {
	...
	private static final String ZULRAH_BEST_TIME_USERNAME_ATTRIBUTE_KEY = "best_zulrah_time_username";
	private static final String ZULRAH_BEST_TIME_TIME_ATTRIBUTE_KEY = "best_zulrah_time_time";

	public static Pair<String, Long> getBestZulrahTime() {
		return Pair.of(Server.getServerAttributes().getString(ZULRAH_BEST_TIME_USERNAME_ATTRIBUTE_KEY),
				Server.getServerAttributes().getLong(ZULRAH_BEST_TIME_TIME_ATTRIBUTE_KEY));
	}

	public static void setBestZulrahTime(String username, long time) {
		Server.getServerAttributes().setString(ZULRAH_BEST_TIME_USERNAME_ATTRIBUTE_KEY, username);
		Server.getServerAttributes().setLong(ZULRAH_BEST_TIME_TIME_ATTRIBUTE_KEY, time);
	}
	...
	public void stop() {
		...
		Pair<String, Long> globalBest = getBestZulrahTime();
		if (globalBest.getLeft() == null || globalBest.getRight() == 0 || time < globalBest.getRight() && globalBest.getRight() != 0) {
			...
			if (globalBest.getLeft() != null && globalBest.getRight() != 0) {
				PlayerHandler.executeGlobalMessage("<img=10></img>[<col=255>News</col>] The old record was set by <col=CC0000>" + globalBest.getLeft()
						+ "</col> with a time of <col=CC0000>" + Misc.toFormattedMS(globalBest.getRight()) + "</col>.");
			}
			setBestZulrahTime(player.playerName, time);
		}
