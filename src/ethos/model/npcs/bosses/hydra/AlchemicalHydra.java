...
public class AlchemicalHydra extends SingleInstancedArea {
    ...
    public static void negateDamage(Player player, NPC npc, Damage damage) {
        if (player.getInstance() != null && player.getInstance() instanceof AlchemicalHydra) {
            ...
            if (instance.npc == npc) {
                if (!instance.sprayed) {
                    damage.setAmount(damage.getAmount() - (damage.getAmount() * 75 / 100));
                }
            }
        }
    }
            ...
            public void execute(CycleEventContainer container) {
                ...
                if (tick == 0) {
                    ...
                    steps.stream().forEach(loc -> {
                    	Location nextStep = npc.getRegionProvider().get(loc.getX(), loc.getY()).getNextStepLocation(loc.getX(), loc.getY(), player.getX(), player.getY(), height, 1, 1);
                        ...
                        if (player.getLocation().equalsIgnoreHeight(loc)) {
                            ...
                            player.appendDamage(Misc.random(6, 13), Hitmark.HIT);
                        }
                    });
                } else if (tick >= 15) {
                    container.stop();
        ...
        CycleEventHandler.getSingleton().addEvent(2, npc, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
                ...
                if (!spawnedFire.isEmpty()) {
                    ...
                    if (inFire) {
                        player.appendDamage(Misc.random(1, 13), Hitmark.HIT);
                    }
                }
                if (tick == 0) {
                    npc.facePlayer(player.getIndex());
        ...
        CycleEventHandler.getSingleton().addEvent(1, npc, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
                ...
                if (Boundary.isIn(player, fireZones[0], (tick >= 10 ? fireZones[1] : Boundary.EMPTY)) && tick >= 4 && tick <= 60) {
                    player.appendDamage(Misc.random(2, 20), Hitmark.HIT);
                }
                tick++;
            }
            ...
            CycleEventHandler.getSingleton().addEvent(npc, new CycleEvent() {
                ...
                public void execute(CycleEventContainer container) {
                    ...
                    if (tick >= 3) {
                        ...
                        if (targetInLocation) {
                            player.appendDamage(Misc.random(1, 14), Hitmark.HIT);
