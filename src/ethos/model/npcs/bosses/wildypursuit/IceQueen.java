...
public class IceQueen {
	...
	public static void queenSpecial(Player player) {
		...
		if (QUEEN.getHealth().getCurrentHealth() < 1400 && specialAmount == 0 ||
			QUEEN.getHealth().getCurrentHealth() < 1100 && specialAmount == 1 ||
			QUEEN.getHealth().getCurrentHealth() < 900 && specialAmount == 2 ||
			QUEEN.getHealth().getCurrentHealth() < 700 && specialAmount == 3 ||
			QUEEN.getHealth().getCurrentHealth() < 400 && specialAmount == 4 ||
			QUEEN.getHealth().getCurrentHealth() < 100 && specialAmount == 5) {
	...
	public static void rewardPlayers(Player player) {
		...
		.forEach(p -> {
				...
				if (p.getIceQueenDamageCounter() >= 1) {
					...
					p.getItems().addItemUnderAnyCircumstance(13438, 1);
				} else {
