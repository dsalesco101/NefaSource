...
public class CorporealBeast {
	...
	public static void checkCore() {
		...
		if (core.stream().filter(Objects::nonNull).anyMatch(n -> n.npcType == 320 && !n.isDead && n.getHealth().getCurrentHealth() > 0)) {
			return;
		}

		int currentHealth = corp.getHealth().getCurrentHealth();
	...
	public static void attack(Player attacker, Damage damage) {
		...
		if (corp.getHealth().getCurrentHealth() > 50)
		if (!attacker.rubyspecial == true)
		if (!attacker.getItems().isWearingItem(11824))
		if (!attacker.getItems().isWearingItem(11889)) {
