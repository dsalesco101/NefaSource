	...
	public static void follow(NPC npc, Entity following) {
		...
		if (distance > 1) {
			...
			if (moves[NORTH] && moves[EAST]) {
				for (Tile tiles : npcTiles) {
					if (npc.getRegionProvider().blockedNorth(tiles.getTileX(), tiles.getTileY(), tiles.getTileHeight())) {
						blocked[0] = true;
					}
					if (npc.getRegionProvider().blockedEast(tiles.getTileX(), tiles.getTileY(), tiles.getTileHeight())) {
						blocked[1] = true;
					}
					if (npc.getRegionProvider().blockedNorthEast(tiles.getTileX(), tiles.getTileY(), tiles.getTileHeight())) {
						blocked[2] = true;
					}
				}
				...
				for (Tile tiles : npcTiles) {
					...
					if (npc.getRegionProvider().blockedWest(tiles.getTileX(), tiles.getTileY(), tiles.getTileHeight())) {
						blocked[1] = true;
					}
					if (npc.getRegionProvider().blockedNorthWest(tiles.getTileX(), tiles.getTileY(), tiles.getTileHeight())) {
						blocked[2] = true;
					}
				}
				...
				for (Tile tiles : npcTiles) {
					if (npc.getRegionProvider().blockedSouth(tiles.getTileX(), tiles.getTileY(), tiles.getTileHeight())) {
						blocked[0] = true;
					}
					if (npc.getRegionProvider().blockedEast(tiles.getTileX(), tiles.getTileY(), tiles.getTileHeight())) {
						blocked[1] = true;
					}
					if (npc.getRegionProvider().blockedSouthEast(tiles.getTileX(), tiles.getTileY(), tiles.getTileHeight())) {
						blocked[2] = true;
					}
				}
				...
				for (Tile tiles : npcTiles) {
					...
					if (npc.getRegionProvider().blockedSouthWest(tiles.getTileX(), tiles.getTileY(), tiles.getTileHeight())) {
						blocked[2] = true;
					}
				}
			...
			for (int index = 0; index < 2; index++) {
				...
				int moveY = 0;
				if (index == 0) {
					if (following.getX() > npc.getX() || following.getX() < npc.getX()) {
						moveY = following.getY() > npc.getY() ? 1 : -1;
					}
				} else {
					if (following.getY() > npc.getY() || following.getY() < npc.getY()) {
						moveX = following.getX() > npc.getX() ? 1 : -1;
					}
				}

				int direction = NPCClipping.getDirection(moveX, moveY);
				if (direction != -1 && npc.getRegionProvider().canMove(npc.absX, npc.absY, npc.heightLevel, direction)) {
					x = moveX;
					y = moveY;
					break;
				}
	...
	public static boolean canMoveTo(final NPC mob, final int direction) {
		...
		for (int i = 1; i < size + 1; i++) {
			for (int k = 0; k < NPCClipping.SIZES[i].length; k++) {
				...
				if (!mob.getRegionProvider().canMove(x3, y3, z, direction)) {
					return false;
				}
				...
				for (int j = 0; j < 8; j++) {
					...
					if (NPCClipping.withinBlock(x5, y5, size, x6, y6)) {
						if (!mob.getRegionProvider().canMove(x3, y3, z, j)) {
							return false;
						}
					}
				}
